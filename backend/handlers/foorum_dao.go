...
func (dao *FoorumDao) list_posts(filter PostFilter) []Post {
	...
	//(SELECT COUNT (1) FROM likes WHERE likes.post_id = posts.id AND likes.comment_id = comments.id AND likes.likes = 'L') AS num_likes,
	//(SELECT COUNT (1) FROM likes WHERE likes.post_id = posts.id AND likes.comment_id = comments.id AND likes.likes = 'D') AS num_dislikes
